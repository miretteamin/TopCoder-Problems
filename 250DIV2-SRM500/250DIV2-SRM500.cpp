...
public:
	int maxTurns(vector <int> cards)
	{
		...
		{
			if (cards[i + 1] == (cards[i] + 1)) i += 1;
			res++;
		}
		if (i == (cards.size() - 1)) res++;
		return res;
	}
};
...
{
	SRMCards srm;
	cout << srm.maxTurns({ 11, 12, 102, 13, 100, 101, 99, 9, 8, 1 });
...
If the pile contains a card with number A+1, that card is removed from the pile.
The game is finished when the pile becomes empty. It's fun to play the game, so Dmitry wants to spend as much time as possible playing it.  You are given a int[] cards containing the numbers written on the cards in the pile before the start of the game. Return the largest possible number of turns in which Dmitry can finish the game.
Definition
    
Class: SRMCards
Method: maxTurns
...
Returns: int
Method signature: int maxTurns(vector <int> cards) (be sure your method is public)
Limits
Time limit (s):
2.000
Memory limit (MB):
